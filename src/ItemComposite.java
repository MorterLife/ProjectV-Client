...
public class ItemComposite extends CacheableNode {
	...
	void decode(Buffer var1) {
		while (true) {
			int var2 = var1.get();//readubyte
			if (var2 == 0) {
				return;
			}

			method_e_void(var1, var2);
		}
	}
	...
	void method_f_void(final ItemComposite itemdef_7_, final ItemComposite itemdef_8_, final int i) {
		try {
			this.anInt441 = itemdef_7_.anInt441 * 1;
	...
	public final OffScreenModel method_m_cy(final int i, final int i_10_) {
		...
		try {
			...
			final OffScreenModel rsmodel_13_ = OffScreenModel.getModel(field_e_150, this.anInt441 * 751244069, 0);
			if (rsmodel_13_ == null) {
				return null;
			}
	...
	public final Model method_a_cg(final int i, final int i_16_) {
		...
		try {
			...
			final OffScreenModel rsmodel = OffScreenModel.getModel(field_e_150, 751244069 * this.anInt441, 0);
			if (rsmodel == null) {
				return null;
			}
	...
	void method_g_void(final ItemComposite itemdef_30_, final ItemComposite itemdef_31_, final int i) {
		try {
			this.anInt441 = 1 * itemdef_30_.anInt441;
...
/*
	void method_e_void(Buffer rsbytebuffer, int opcode) {		
		if (opcode == 1) {
			this.anInt441 = rsbytebuffer.getShort();
		} else if (opcode == 2) {
			name = rsbytebuffer.getString();
		} else if (opcode == 4) {
			field_v_120 = rsbytebuffer.getShort();
		} else if (opcode == 5) {
			xan2d = rsbytebuffer.getShort();
		} else if (opcode == 6) {
			yan2d = rsbytebuffer.getShort();
		} else if (opcode == 7) {
			xOf2d = rsbytebuffer.getShort();
			if (xOf2d > 32767) {
				xOf2d -= 65536;
			}
		} else if (opcode == 8) {
			yOf2d = rsbytebuffer.getShort();
			if (yOf2d > 32767) {
				yOf2d -= 65536;
			}
		} else if (opcode == 11) {
			...
			cost = rsbytebuffer.getInt();
		} else if (opcode == 16) {
			members = true;
		} else if (opcode == 23) {
			this.field_ax_131 = rsbytebuffer.getShort();
			this.field_ar_133 = rsbytebuffer.get();
		} else if (opcode == 24) {
			this.field_al_106 = rsbytebuffer.getShort();
		} else if (opcode == 25) {
			this.field_as_134 = rsbytebuffer.getShort();
			this.field_ao_136 = rsbytebuffer.get();
		} else if (opcode == 26) {
			this.field_ab_144 = rsbytebuffer.getShort();
		} else if (opcode >= 30 && opcode < 35) {
			options[opcode - 30] = rsbytebuffer.getString();
			if (options[opcode - 30].equalsIgnoreCase("Hidden")) {
				options[opcode - 30] = null;
			}
		} else if (opcode >= 35 && opcode < 40) {
			zoom2d[opcode - 35] = rsbytebuffer.getString();
		} else {
			int var3;
			int var4;
			if (opcode == 40) {
				var3 = rsbytebuffer.get();
				this.recol_s = new short[var3];
				this.recol_d = new short[var3];

				for (var4 = 0; var4 < var3; var4++) {
					this.recol_s[var4] = (short) rsbytebuffer.getShort();
					this.recol_d[var4] = (short) rsbytebuffer.getShort();
				}
			} else if (opcode == 41) {
				var3 = rsbytebuffer.get();
				...
				for (var4 = 0; var4 < var3; var4++) {
					this.retex_s[var4] = (short) rsbytebuffer.getShort();
					this.retex_d[var4] = (short) rsbytebuffer.getShort();
				}
			} else if (opcode == 42) {
				anInt443 = rsbytebuffer.readByte();
			} else if (opcode == 65) {
				field_av_153 = true;
			} else if (opcode == 78) {
				this.field_ac_105 = rsbytebuffer.getShort();
			} else if (opcode == 79) {
				this.field_az_138 = rsbytebuffer.getShort();
			} else if (opcode == 90) {
				this.field_at_139 = rsbytebuffer.getShort();
			} else if (opcode == 91) {
				this.field_an_141 = rsbytebuffer.getShort();
			} else if (opcode == 92) {
				this.field_aa_140 = rsbytebuffer.getShort();
			} else if (opcode == 93) {
				this.field_aw_157 = rsbytebuffer.getShort();
			} else if (opcode == 95) {
				zan2d = rsbytebuffer.getShort();
			} else if (opcode == 97) {
				field_aq_125 = rsbytebuffer.getShort();
			} else if (opcode == 98) {
				field_ay_146 = rsbytebuffer.getShort();
				...
				this.field_ai_143[opcode - 100] = rsbytebuffer.getShort();
				this.field_ah_129[opcode - 100] = rsbytebuffer.getShort();
			} else if (opcode == 110) {
				this.sizeX = rsbytebuffer.getShort();
			} else if (opcode == 111) {
				this.sizeY = rsbytebuffer.getShort();
			} else if (opcode == 112) {
				this.sizeZ = rsbytebuffer.getShort();
			} else if (opcode == 113) {
				ambience = rsbytebuffer.readByte();
			} else if (opcode == 114) {
				contrast = rsbytebuffer.readByte() * 5;
			} else if (opcode == 115) {
				team = rsbytebuffer.get();
			} else if (opcode == 139) {
				this.field_bs_154 = rsbytebuffer.getShort();
			} else if (opcode == 140) {
				this.field_bo_155 = rsbytebuffer.getShort();
			} else if (opcode == 148) {
				anInt445 = rsbytebuffer.getShort();
			} else if (opcode == 149) {
				anInt467 = rsbytebuffer.getShort();
			}
		}

	}*/
	
	void method_e_void(final Buffer rsbytebuffer, final int i) {
		try {
			if (i == 1) {
				this.anInt441 = rsbytebuffer.getShort() * -1222455123;
			} else if (i == 2) {
				name = rsbytebuffer.getString();
			} else if (i == 4) {
	...
	public int method499() {
		return anInt443 != -1 && zoom2d != null ? anInt443 >= 0 ? zoom2d[anInt443] != null ? anInt443 : -1
				: "Drop".equalsIgnoreCase(zoom2d[4]) ? 4 : -1 : -1;
	}
