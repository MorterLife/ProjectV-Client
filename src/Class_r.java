	...
	public static void initCacheDirectory(final String string, final String string_3_, final int i, final int i_4_, final int i_5_) throws IOException {
		try {
			...
			while_5_: while (i_6_ < 4) {
				...
				if (Class_ge.field_y_2979.exists()) {
					try {
						...
						if (i_10_ <= 2) {
							...
							string_8_ = System.getProperty("user.home")+"/.ProjectV/oldschool/LIVE";//= rsbytebuffer.method_ae_String((byte) -8);
							if (i_11_ == 1) {
				...
				if (string_8_ == null) {
					string_8_ = new StringBuilder(Class_ed.cache_dir).append(File.separatorChar).append(".ProjectV").append(string_7_).append(File.separatorChar).append(string).append(File.separatorChar).append(string_3_).append(File.separatorChar).toString();
					bool = true;
				}
				if (string_9_ != null) {
